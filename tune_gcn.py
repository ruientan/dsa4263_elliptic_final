"""
Hyperparameter tuning for GCN on the engineered_weight setting.
"""
...
# ---------- Core: train a single GCN with given hyperparams ----------
def train_gcn_with_params(
    data,
    hidden_dim=128,
    lr=0.01,
    weight_decay=5e-4,
    ...
    """
    Train GCN with a specific set of hyperparameters.
    Returns:
      - best_val_f1
      - test_metrics (dict)
      - epochs_trained
    ...
    for epoch in range(epochs):
        ...
        if epoch % 5 == 0:
            ...
            # Early stopping (same logic as experiment suite)
...
def main():
    ...
    # 2. Define a SMALL hyperparameter grid (similar to GraphSAGE)
    hidden_dims = [64, 128]
    lrs = [0.005, 0.01]
    ...
    for i, (hidden_dim, lr, wd) in enumerate(search_space, start=1):
        ...
        val_f1, test_metrics, epochs_trained = train_gcn_with_params(
            data,
            hidden_dim=hidden_dim,
            lr=lr,
            weight_decay=wd,
    ...
    csv_path = "results/gcn_tuning_results.csv"
    ...
    json_path = "results/gcn_tuning_results.json"
    ...
    print("\n" + "=" * 80)
    print("Best GCN config (by validation F1)")
    print("=" * 80)
    print(best_config)
